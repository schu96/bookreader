...
export class OlPopover extends LitElement {
    static properties = {
      for: {type: String}, // id of external trigger via getElementsById
    ...
    updated(changed) {
      ...
      } else if (changed.has('anchor') && this.open && !this._mobile) {
        // Retargeted to a new anchor while already open (e.g. re-triggered
        // from a different menu option) — reposition without re-animating.
    ...
    get _anchorEl() {
      return this._externalTriggerEl ?? this._triggerEl;
    }
    ...
    _onOutsideClick(e) {
      ...
      const ext = this._externalTriggerEl;
      if (!path.includes(this) && !(ext && path.includes(ext))) {
